 ...
 */
define( 'QUILLWORK_WELCOME_FLAG', 'quillwork_welcome_notice' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
define( 'QUILLWORK_DISMISS_ACTION', 'quillwork_dismiss_welcome' ); // hardcoded slug — CLI rewrites on re-skin.
...
function quillwork_get_onboarding_capability(): string {
	 ...
	 */
	return (string) apply_filters( QUILLWORK_SLUG . '/onboarding_capability', 'edit_theme_options' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function quillwork_get_started_content(): array {
	...
	$default = array(
		 ...
		 * echo in quillwork_render_get_started_page() (the 'title' at the <strong>, the
		...
		'optimize'   => array(
			esc_html__( "This theme is fast by design: reading is JavaScript-free (only the breaking-news dismiss control ships a small script), self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'quillwork' ),
			esc_html__( 'It is built to WCAG 2.2 AA guidance — real focus outlines, a skip link, sensible heading order, and motion that respects a reduce-motion setting. Keep your own copy and images to that bar and the whole site stays welcoming.', 'quillwork' ),
		),
		'developers' => array(
			...
			'url'   => apply_filters( QUILLWORK_SLUG . '/developer_guide_url', 'https://thisismyurl.com/quillwork/' ), // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 */
	return (array) apply_filters( QUILLWORK_SLUG . '/get_started_content', $default ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
